...
const EditTask = ({ isOpen, onCancel, task, otherAction, optionalID }) => {
  if (!isOpen) return null;
  const validationSchema = yup.object({
    name: yup.string().required("Please proide task name"),
    description: yup.string().required("Please proide task description"),
  });
  const formik = useFormik({
    ...
    initialValues: {
      ...
      projectID: task?.project?._id || optionalID,
    },
    validationSchema,
    onSubmit: async (values, { setSubmitting }) => {
      setSubmitting(true);
      try {
        const res = await api.patch(`/user/updateTask/${task._id}`, values);
        if (res.status === 200) {
        ...
        // console.log("errorUpdatingTask", error);
        const errMessage =
